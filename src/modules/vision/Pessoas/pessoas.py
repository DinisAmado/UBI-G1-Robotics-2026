...
class SpeakerTracker:
    def __init__(self):
        ...
        self.FAST_ALPHA = 0.60
        self.SLOW_ALPHA = 0.05
        self.SPEAK_DELTA = 0.015

    def push(self, pid: str, mar: float):
        if pid not in self._fast_ema:
            self._fast_ema[pid] = mar
            self._slow_ema[pid] = mar
        else:
            self._fast_ema[pid] = (self.FAST_ALPHA * mar) + (
                (1 - self.FAST_ALPHA) * self._fast_ema[pid]
            )
            self._slow_ema[pid] = (self.SLOW_ALPHA * mar) + (
                (1 - self.SLOW_ALPHA) * self._slow_ema[pid]
            )

    def get_delta(self, pid: str) -> float:
        if pid not in self._fast_ema:
            return 0.0
        return max(0.0, self._fast_ema[pid] - self._slow_ema[pid])

    def is_speaking(self, pid: str) -> bool:
        return self.get_delta(pid) > self.SPEAK_DELTA
...
class G1InsightFacePerception:
    ...
    def run(self):
        ...
        while cap.isOpened():
            ...
            cv2.putText(
                frame,
                f"DB: {len(self.face_db.db)} IDS | FPS: {fps:.1f} | CAM {self.camera_index} | Apple M4",
                (10, 60),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                (0, 255, 255),
