...
# Audio via microfone local (fallback quando ZMQ nao funciona)
MIC_DEVICE_INDEX = None   # None = microfone padrao
MIC_THRESHOLD    = 1500   # aumenta se ouvir ruido

# Audio via ZMQ do robo (quando o stream ZMQ estiver disponivel)
...
# ==============================================================================
# AUDIO — MICROFONE LOCAL (funciona sempre)
# ==============================================================================
def gravar_microfone() -> Optional[str]:
    r = sr.Recognizer()
    ...
    with sr.Microphone(device_index=MIC_DEVICE_INDEX) as source:
        print("\n[MIC] A ouvir... (microfone local)")
        try:
            audio = r.listen(source, timeout=10, phrase_time_limit=15)
            with open(AUDIO_TEMP, "wb") as f:
                f.write(audio.get_wav_data())
            print("[MIC] Gravacao concluida.")
            return AUDIO_TEMP
        except sr.WaitTimeoutError:
            print("[MIC] Timeout — ninguem falou.")
            return None

# ==============================================================================
# AUDIO — ZMQ DO ROBO (quando o stream estiver disponivel)
# ==============================================================================
...
def gravar_zmq() -> Optional[str]:
    ctx = zmq.Context()
    sock = ctx.socket(zmq.SUB)
    sock.setsockopt(zmq.RCVTIMEO, ZMQ_TIMEOUT * 1000)  # timeout ANTES do connect
    ...
    print("\n[MIC] A ouvir... (microfone ZMQ do robo)")
    try:
        while True:
            try:
                ...
                if speech_started and speech_duration >= VAD_MIN_SPEECH_SECS:
                    ...
                    print("[MIC] Sem sinal ZMQ.")
                    return None  # nao ha stream, sai imediatamente
...
def gravar(usar_zmq: bool) -> Optional[str]:
    """Escolhe automaticamente ZMQ ou microfone local."""
    if usar_zmq:
        return gravar_zmq()
    else:
        return gravar_microfone()

# ==============================================================================
# TTS — ALTIFALANTES DO ROBO
# ==============================================================================
...
def falar(texto, audio_client):
    ...
    subprocess.run([
        ...
        "-ar", "16000", "-ac", "1", "-sample_fmt", "s16", AUDIO_MONO
    ...
    try:
        ...
        print("[TTS] Audio reproduzido no robo")
    except Exception as e:
...
# ==============================================================================
def main():
    # Argumento opcional: --mic para forcar microfone local
    usar_zmq = "--mic" not in sys.argv
    ...
    modo = "ZMQ (microfone do robo)" if usar_zmq else "microfone local"
    print("=" * 52)
    print(f"   SISTEMA HRI -- UNITREE G1  [{modo}]")
    print("=" * 52 + "\n")

    try:
        while True:
            ficheiro = gravar(usar_zmq)
            if not ficheiro:
                continue
            try:
