...
def falar(texto, audio_client):
    ...
    # 2. Converter MP3 -> WAV 16kHz mono (formato exigido pelo AudioClient)
    subprocess.run([
        "ffmpeg", "-y", "-i", AUDIO_RESP,
        "-ar", "16000", "-ac", "1", AUDIO_MONO
    ], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

    # 3. Tocar nos altifalantes do robô
    pcm_list, sample_rate, num_channels, is_ok = read_wav(AUDIO_MONO)
    if is_ok and sample_rate == 16000 and num_channels == 1:
        audio_client.SetVolume(100)
        play_pcm_stream(audio_client, pcm_list, "hri_response")
        audio_client.PlayStop("hri_response")
    else:
        print("[ERRO] WAV com formato inesperado")
