...
Audio:         ZMQ streaming com VAD (Voice Activity Detection)
...
PORT           = 5556
ZMQ_TIMEOUT    = 5           # segundos a esperar por pacote ZMQ antes de desistir

# VAD — ajusta estes dois valores se necessario
VAD_THRESHOLD       = 500    # RMS minimo para considerar que ha voz (aumenta se ouvir ruido de fundo)
VAD_SILENCE_SECS    = 1.2    # segundos de silencio apos fala para comecar a processar
VAD_MIN_SPEECH_SECS = 0.4    # duracao minima de fala para nao ignorar (evita cliques e ruidos curtos)
...
# ==============================================================================
# 4. AUDIO — ZMQ com VAD (Voice Activity Detection)
# ==============================================================================
def calcular_rms(pcm_bytes: bytes) -> float:
    """Calcula o volume RMS de um chunk de audio PCM int16."""
    if len(pcm_bytes) < 2:
        return 0.0
...
def gravar() -> Optional[str]:
    """
    Escuta continuamente o microfone do G1 via ZMQ.
    - Comeca a gravar quando deteta voz (RMS > VAD_THRESHOLD)
    - Para de gravar e processa quando deteta silencio (VAD_SILENCE_SECS)
    - Ignora ruidos muito curtos (< VAD_MIN_SPEECH_SECS)
    """
    ctx = zmq.Context()
    ...
    audio_buffer = bytearray()  # buffer so com os chunks de fala
    last_sr = 48000
    last_ch = 1

    speech_started = False
    speech_duration = 0.0     # segundos de fala acumulados
    silence_duration = 0.0    # segundos de silencio consecutivos apos fala

    print("\n[MIC] A ouvir... (fala quando quiseres)")
    ...
    try:
        while True:
            try:
                ...
                # timeout ZMQ — se ja havia fala suficiente, processa
                if speech_started and speech_duration >= VAD_MIN_SPEECH_SECS:
                    print("[MIC] Timeout ZMQ apos fala, a processar...")
                    break
                print("[MIC] Sem sinal do robo, a tentar de novo...")
                continue
            ...
            # Duracao deste chunk em segundos
            chunk_secs = (len(pcm) // 2) / last_sr  # samples / sample_rate
            ...
            if is_speech:
                if not speech_started:
                    speech_started = True
                    print("[MIC] Voz detetada, a gravar...")
                ...
                if speech_started:
                    ...
                    audio_buffer.extend(pcm)  # inclui o silencio final (ajuda o Whisper)

                    if silence_duration >= VAD_SILENCE_SECS:
                        if speech_duration >= VAD_MIN_SPEECH_SECS:
                            print(f"[MIC] Silencio detetado ({silence_duration:.1f}s), a processar...")
                            break
                        else:
                            # fala muito curta, provavelmente ruido — reinicia
                            print("[MIC] Ruido ignorado, a continuar a ouvir...")
                            audio_buffer.clear()
                            speech_started = False
                            speech_duration = 0.0
                            silence_duration = 0.0
